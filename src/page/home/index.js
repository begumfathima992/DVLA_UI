...
function ServiceCard({ icon, title, desc, price, index }) {
  return (
    <FadeUp delay={index * 0.08}>
      <motion.div
        whileHover={{ y: -6, borderColor: "rgba(220,38,38,0.5)" }}
        transition={{ type: "spring", stiffness: 300, damping: 20 }}
        className="group bg-zinc-900 border border-zinc-800 rounded-2xl p-6 cursor-pointer relative overflow-hidden h-full"
      >
        {/* Hover glow */}
        <div className="absolute inset-0 bg-gradient-to-br from-red-950/0 to-red-950/0 group-hover:from-red-950/20 group-hover:to-transparent transition-all duration-500 rounded-2xl pointer-events-none" />
        ...
        <div className="flex items-center justify-between mt-auto">
          ...
          <motion.span
            className="text-zinc-600 group-hover:text-red-500 text-lg"
            animate={{ x: 0 }}
            whileGroupHover={{ x: 4 }}
          >
            →
...
function TestiCard({ stars, text, name, initials, car, delay }) {
  return (
    <FadeUp delay={delay}>
      <div className="bg-zinc-900 border border-zinc-800 hover:border-zinc-700 transition-colors rounded-2xl p-6 h-full">
        ...
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-full bg-red-950 border border-red-800 flex items-center justify-center text-xs font-bold text-red-400 flex-shrink-0">
            {initials}
          </div>
          <div>
            <div className="text-white text-sm font-semibold">{name}</div>
            <div className="text-zinc-600 text-xs">{car}</div>
          </div>
        </div>
      </div>
    </FadeUp>
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-white overflow-x-hidden">
      ...
      <section className="relative min-h-screen flex items-center overflow-hidden bg-black pt-16">
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center min-h-[90vh]">
            ...
            <div className="space-y-8">
              ...
              <motion.div
                ...
                className="inline-flex items-center gap-2 bg-red-900/30 border border-red-700 rounded-full px-6 py-2 text-sm font-semibold tracking-widest text-red-400"
              >
              ...
              <motion.h1
                ...
                className="font-black  leading-[0.99] text-7xl lg:text-[5.8rem]"
                style={{ fontFamily: "'Bebas Neue', sans-serif" }}
              >
                YOUR CAR.
                <br />
                <span className="text-red-500">OUR CRAFT.</span>
                <br />
                <span className="text-zinc-500">ZERO LIMITS.</span>
              </motion.h1>
              ...
              >
                <motion.button
                  ...
                  className="flex items-center gap-3 bg-red-600 hover:bg-red-500 px-8 py-4 rounded-2xl font-bold text-lg transition-all"
                >
                  <svg
                ...
                <motion.button
                  whileHover={{ scale: 1.05 }}
                  whileTap={{ scale: 0.95 }}
                  className="flex items-center gap-3 border border-zinc-700 hover:border-zinc-500 px-8 py-4 rounded-2xl font-bold text-lg text-zinc-300 hover:text-white transition-all"
                >
                  📅 Book Service
                </motion.button>
              </motion.div>
            ...
            {/* Right Side - Image (Perfectly Integrated) */}
            <motion.div
            ...
            >
              <div className="relative rounded-xl overflow-hidden ">
                ...
                {/* Dark overlay for better text readability & mood */}
                <div className="absolute inset-0 bg-gradient-to-r from-black/40 via-transparent to-black/30" />

                {/* Professional Service Badge */}
                <div className="absolute top-6 right-6 bg-black/80 backdrop-blur-md px-5 py-2.5 rounded-2xl text-sm font-semibold border border-red-500/30 text-white">
        ...
        >
          <span className="text-zinc-500 text-xs tracking-[3px] uppercase">
            Scroll to explore
          </span>
          <svg
        ...
        <div className="max-w-6xl mx-auto px-6 grid grid-cols-2 md:grid-cols-4">
          {[
            ...
            <FadeUp key={s.label} delay={i * 0.1}>
              <div className="py-10 px-6 text-center border-r border-zinc-800 last:border-r-0">
                <div
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {[
                  ...
                  <div key={label}>
                    ...
                    <input
                      placeholder={ph}
                      className="w-full bg-zinc-800 border border-zinc-700 focus:border-red-500 rounded-xl px-4 py-3 text-sm text-white outline-none transition-colors placeholder-zinc-600"
                    />
                  </div>
                ))}
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                {[
                  ...
                  <div key={label}>
                    ...
                    <select className="w-full bg-zinc-800 border border-zinc-700 focus:border-red-500 rounded-xl px-4 py-3 text-sm text-white outline-none transition-colors appearance-none">
                      {opts.map((o) => (
                ...
                <div>
                  ...
                  <input
                    type="date"
                    className="w-full bg-zinc-800 border border-zinc-700 focus:border-red-500 rounded-xl px-4 py-3 text-sm text-white outline-none transition-colors"
                  />
                </div>
              </div>
              <motion.button
                ...
                whileTap={{ scale: 0.98 }}
                className="w-full bg-red-600 hover:bg-red-500 text-white font-bold py-4 rounded-xl text-base transition-colors mt-2"
              >
                Confirm Booking →
              </motion.button>
            </div>
          ...
          {[
            ...
            <FadeUp key={w.n} delay={i * 0.1}>
              <motion.div
                ...
                className="bg-zinc-900 border border-zinc-800 hover:border-zinc-700 rounded-2xl p-6"
              >
                <div
                  className="text-5xl font-black text-red-800/50 mb-4"
                  style={{ fontFamily: "'Bebas Neue', cursive" }}
                >
                  {w.n}
                </div>
      ...
      <section className="py-24 max-w-4xl mx-auto px-6 text-center">
        <FadeUp>
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.96 }}
            onClick={() => setPage && setPage("search")}
            className="inline-flex items-center gap-3 bg-red-600 hover:bg-red-500 text-white font-black px-12 py-5 rounded-2xl text-xl transition-colors"
            style={{
          ...
          <p className="text-zinc-700 text-xs mt-5 font-mono">
            Try: YY59UAD · MH12AB1234 · UP32CD5678
          </p>
        </FadeUp>
      </section>
      ...
      <footer className="border-t border-zinc-800 py-10 text-center">
        <div
          ...
          style={{ fontFamily: "'Bebas Neue', cursive" }}
        >
          AUTO<span className="text-red-500">FORGE</span> GARAGE
        </div>
